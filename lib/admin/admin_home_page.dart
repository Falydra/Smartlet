    ...
    try {
      ...
      if (harvestResult['success'] == true) {
        setState(() {
          ...
          _harvestCount =
              harvestResult['meta']?['total'] ?? _recentHarvests.length;
        });
      }
          ...
          : SingleChildScrollView(
              child: Padding(
                padding: EdgeInsets.symmetric(
                    horizontal: width(context) * 0.05, vertical: 16),
                child: Column(
                        ...
                        child: Column(
                          ...
                          children: [
                            const Row(
                              children: [
                                Icon(Icons.settings,
                                    color: Color(0xFF245C4C), size: 24),
                                SizedBox(width: 8),
                                Text(
                                  'Admin Tools',
                                  style: TextStyle(
                            ...
                            ListTile(
                              ...
                              subtitle:
                                  const Text('View and manage harvest records'),
                              trailing: const Icon(Icons.chevron_right),
                              onTap: () => Navigator.pushNamed(
                                  context, '/admin-harvest'),
                            ),
                            const Divider(height: 1),
                            ListTile(
                              dense: true,
                    ...
                    if (_recentHarvests.isNotEmpty) ...[
                      _buildSectionCard(
                        ...
                        children: _recentHarvests.map((harvest) {
                          final rbwName =
                              harvest['rbw']?['name']?.toString() ?? 'Unknown';
                          final nestsCount =
                              harvest['nests_count']?.toString() ?? '0';
                          final weightKg =
                              harvest['weight_kg']?.toString() ?? '0';
                          return ListTile(
                            dense: true,
                            leading: const Icon(Icons.agriculture, size: 20),
                            title: Text(rbwName,
                                style: const TextStyle(fontSize: 14)),
                            subtitle: Text('$nestsCount nests, $weightKg kg',
                                style: const TextStyle(fontSize: 12)),
                            trailing: const Icon(Icons.chevron_right, size: 16),
                          );
                        }).toList(),
